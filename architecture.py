...
def get_architecture(arch, hidden, dr, channels=1):
    ...
    elif arch=='o3_512_err': return model_o3_512_err(hidden, dr, channels)
    elif arch=='o3_128_err': return model_o3_128_err(hidden, dr, channels)
...
#####################################################################################
#####################################################################################
class model_o3_128_err(nn.Module):
    def __init__(self, hidden, dr, channels):
        super(model_o3_128_err, self).__init__()
        ...
        self.FC1  = nn.Linear(64*hidden, 32*hidden)  
        self.FC2  = nn.Linear(32*hidden, 2) # 12)  
...
#####################################################################################
#####################################################################################
class model_o3_512_err(nn.Module):
    def __init__(self, hidden, dr, channels):
        super(model_o3_512_err, self).__init__()
        ...
        # input: 32*hiddenx8x8 ----------> output:64*hiddenx4x4
        self.C61 = nn.Conv2d(64*hidden, 128*hidden, kernel_size=3, stride=1, padding=1,
                            padding_mode='circular', bias=True)
        self.C62 = nn.Conv2d(128*hidden, 128*hidden, kernel_size=3, stride=1, padding=1,
                            padding_mode='circular', bias=True)
        self.C63 = nn.Conv2d(128*hidden, 128*hidden, kernel_size=2, stride=2, padding=0,
                            padding_mode='circular', bias=True)
        self.B61 = nn.BatchNorm2d(128*hidden)
        self.B62 = nn.BatchNorm2d(128*hidden)
        self.B63 = nn.BatchNorm2d(128*hidden)

        # input: 64*hiddenx4x4 ----------> output: 128*hiddenx1x1
        self.C71 = nn.Conv2d(128*hidden, 256*hidden, kernel_size=4, stride=1, padding=0,
                            padding_mode='circular', bias=True)
        self.B71 = nn.BatchNorm2d(256*hidden)

        self.P0  = nn.AvgPool2d(kernel_size=2, stride=2, padding=0)

        self.FC1  = nn.Linear(256*hidden, 64*hidden)  
        self.FC2  = nn.Linear(64*hidden, 2) # 12)  
    ...
    def forward(self, image):
        ...
        x = self.LeakyReLU(self.B61(self.C61(x)))
        x = self.LeakyReLU(self.B62(self.C62(x)))
        x = self.LeakyReLU(self.B63(self.C63(x)))

        x = self.LeakyReLU(self.B71(self.C71(x)))
