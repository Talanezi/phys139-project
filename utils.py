...
def best_params_database(study_name, storage):
    ...
    print("Study Name", study_name, 'Storage', storage)
    ...
    indexes = np.argsort(values) #gives best trial
    #indexes = [40] #put which trial you want #JONAH EDIT
    for i in [0]: #range(1):
...
def best_model_params(study_name, storage, fields, root, label, errors=False, 
                      sim='IllustrisTNG', monopole=True, suite=''):
    ...
    if errors is False:
        if label is None:
            ...
            fmodel = '%s/models/model%s_cosmo_%s_%s_%s.pt'%(root, suffix, num, label, study_name)
    elif errors is True:
        if label is None:
            fmodel = '%s/models/model%s_%s_errors.pt'%(root, suffix, num)
        else:
            fmodel = '%s/models/model%s_%s_%s_%s_errors.pt'%(root, suffix, num, label, study_name)
    elif errors=='new':
        if label is None:
            fmodel = '%s/models_%s_%s/model%s_%s.pt'%(root, sim, suite, suffix, num)
        else:
            fmodel = '%s/models_%s_%s/model%s_%s_%s_%s.pt'%(root, sim, suite, suffix, num, label, study_name)
...
def fname_maps(root, fields, sim, z, suite='LH', sim_norm=None):
    ...
    for field in fields:
        ...
        if sim_norm is None:
            if True: # suite=='LH': #JONAH EDIT
                f_maps_norm.append(None)
            else:
                f_maps_norm.append('%s/Images_%s_%s_%s_z=%.2f.npy'%(root,field,sim,suite,z))
        else:
            f_maps_norm.append('%s/Images_%s_%s_%s_z=%.2f.npy'%(root,field,sim_norm,suite,z))
